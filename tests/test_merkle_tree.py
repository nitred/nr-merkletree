...
def test_basic():
    ...
    assert merkle_tree.tree.get_node(nid=expected_hash1).identifier == expected_hash1
    assert merkle_tree.tree.get_node(nid=expected_hash01).identifier == expected_hash01
    assert merkle_tree.tree.get_node(nid=expected_hash2x).identifier == expected_hash2x
    assert merkle_tree.tree.get_node(nid=expected_root_hash).identifier == expected_root_hash

    # Sanity check.
    with pytest.raises(AttributeError):
        assert merkle_tree.tree.get_node(nid=b'not_an_expected_hash').identifier
